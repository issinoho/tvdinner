...
def save_playback_positions(path: Path, positions: dict[str, float]) -> None:
    """Persist {recording path: position in seconds}, dropping any entry
    whose file no longer exists -- otherwise this would grow forever as
    old recordings get deleted (see the 'd' recordings-browser key).
    Creates the parent directory if needed."""
    pruned = {p: seconds for p, seconds in positions.items() if Path(p).exists()}
