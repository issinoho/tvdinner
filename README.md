...
| `g` | While the Plex library browser is open: switch between List view (the default -- a scrolling row per item) and Grid view (large poster tiles, `columns` at a time). The chosen view persists as you browse -- drilling into a show/season, searching, filtering by year, etc. all stay in whichever view you last picked, until you press `g` again. In Grid view, `LEFT`/`RIGHT` move across columns instead of `LEFT` going back a level -- use `ESC`/`GO_BACK` for that there instead; List view is unaffected. |
