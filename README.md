...
| `l` | [Plex](#plex-media-server) sessions only: (re)open the library browser -- `UP`/`DOWN`/`PGUP`/`PGDWN` to move the selection, `ENTER` to drill into a library/show/season or play a movie/episode, `ESC`/`LEFT` to go back a level (or close it, from the top level). While showing a listing of movies or shows specifically (not the top-level library list, and not seasons/episodes), any letter or digit jumps to the next title starting with it (press again to cycle to the next match) -- note this means the other single-letter Plex bindings below (`h`, `v`, `g`, `l`, `y`, ...) are temporarily shadowed at that level, the same trade-off the guide's `f` filter and Plex's own `/` search already make. |
