...
| `r` / `0x211246` | Toggle recording the current stream to disk as a raw copy (no re-encoding), saved under `--record-dir` as `<channel>_<timestamp>.ts`. `0x211246` is a real remote's dedicated Record button -- mpv has no proper symbolic name for it, so it reports as a raw hex key name (see mpv's own "Key names" manual section); unlike `r` itself, it's never shadowed while a guide filter/Plex search/Plex year text-entry prompt is open. |
