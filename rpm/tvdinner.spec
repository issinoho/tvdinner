...
- Bookmarks browser: the TMDB column is replaced by a "tvtimes" one with
  ...
  --report-watch-state and --sync-favourites together.
- Bookmarks can carry a device_name, passed as --device-name on a
  checked tvtimes launch. Never guessed from the hostname.
- The device label is truncated to 120 characters before reporting.
...
- New `tvtimes://` source scheme for the companion tvtimes web app:
  ...
  whole merged line-up with its corrected guide. Sugar rather than a new
  protocol -- it expands to tvtimes' two export feeds, so the guide,
  favorites, recording, scheduling and bookmarks behave as they do for
  any M3U + XMLTV pair.
- The desktop entry claims x-scheme-handler/tvtimes(s) too, so tvtimes'
  "Open in tvdinner" button hands a whole account over in one click.
...
- Manage saved bookmarks non-interactively with `tvdinner bookmarks
  ...
  tool to register a source (e.g. a tvtimes export's merged M3U + XMLTV)
  as a bookmark without the curses picker. `edit`/`remove` take a
  bookmark name or its 1-based position from `list`; `list` alone masks
  login credentials and hides the TMDB token, `list --json` emits the
  raw bookmarks.json. `add` refuses a name that's already taken unless
  `--replace`. Bookmark writes are now atomic.

* Tue Sep 01 2026 Iain Smith <iain@issinoho.com> - 1.31.1-1
- Fix the Plex "Up Next" end-of-episode countdown card never being
  visible -- it was composited behind its own full-screen backdrop, so
  an episode ending showed only the backdrop and logo before the next
  episode auto-played, with no countdown prompt or timer in between.
...
- Fix a live channel's "now playing" hero/banner showing no release
  ...
  FastChannels-generated guide) -- falls back to TMDB, the same match
  rating/director already used

* Fri Aug 28 2026 Iain Smith <iain@issinoho.com> - 1.30.0-1
- Show a context-sensitive item count in the Plex browser header --
  "(x movies)"/"(x shows)"/"(x episodes)"/"(x Seasons)" for a listing
  one level below a library or Continue Watching, no suffix at all for
  the root library list itself

* Fri Aug 28 2026 Iain Smith <iain@issinoho.com> - 1.29.3-1
- Fix chapter thumbnails showing a real but blank-looking frame for TV
  rips whose chapter markers land exactly on a fade-to-black
  transition cut -- the local frame grab now seeks a couple of seconds
  past a chapter's start instead of its exact boundary
- Raise the chapter-thumbnail capture timeout 15s -> 30s -- a slower
  debrid source measured 10-15s per successful grab, occasionally
  longer

* Fri Aug 28 2026 Iain Smith <iain@issinoho.com> - 1.29.2-1
- Fix chapter thumbnails staying permanently blank for the rest of a
  session after failing to generate once -- a failed fetch was being
  cached exactly as permanently as a successful one, blocking any
  future retry even though a fresh attempt would often work
...
- Fix the Plex browser's "Added/Removed from favorites" message
  ...
  into its short display time before the frame reached the screen)
- Fix the Plex browser's title logo showing the wrong movie/show
  after favoriting an item and switching to the favorites-only view
  -- it now matches the backdrop, which was always correct
...
- Fix a Plex episode in a flat listing (search results, Continue
  ...
  always uses its own show, not whatever was browsed earlier
...
- Play a Plex show's theme music while browsing its library page,
  ...
  or picking something to actually watch; see --no-plex-theme-music

* Wed Aug 26 2026 Iain Smith <iain@issinoho.com> - 1.25.2-1
- Show the Plex root wallpaper (instead of mpv's own idle-screen logo)
  behind the "Up Next" countdown card

* Wed Aug 26 2026 Iain Smith <iain@issinoho.com> - 1.25.1-1
- Sharpen the Plex library browser's background wallpaper blur --
  previous value read as noticeably blurry on a real screen

* Wed Aug 26 2026 Iain Smith <iain@issinoho.com> - 1.25.0-1
- Add watching-activity stats to 'tvdinner stats' -- total watch time
  this week/month/all-time, split by live channel/VOD/recording, plus
  the most-watched live channels this month and all-time

* Tue Aug 25 2026 Iain Smith <iain@issinoho.com> - 1.24.0-1
- Add --no-tmdb-cache/--refresh-tmdb-cache, mirroring the existing EPG
  cache flags, so a bad cached TMDB match doesn't have to wait out its
  30-day TTL
- Expire a VOD resume position after 90 days of nobody resuming or
  updating it, instead of keeping it forever
...
- Add --audio-passthrough, --audio-downmix-boost, and
  ...
  same shape as --interpolation/--glsl-shader

* Mon Aug 24 2026 Iain Smith <iain@issinoho.com> - 1.21.0-1
- Add a sleep timer ('e' cycles Off/15/30/60/90 minutes) and document
  the playback-speed/audio-sync/zoom-pan controls mpv already provided
  by default but the app never mentioned anywhere

* Mon Aug 24 2026 Iain Smith <iain@issinoho.com> - 1.20.1-1
- Let ENTER confirm the skip-intro/credits prompt, not just 'j' -- an
  IR/BLE air-mouse remote's OK button sends ENTER, not an arbitrary
  letter, so the prompt was previously unreachable from one. ENTER's
  own "toggle pause" meaning is only shadowed while the prompt is
  showing, restored the moment it closes

* Mon Aug 24 2026 Iain Smith <iain@issinoho.com> - 1.20.0-1
- Add richer technical detail to the 'i' overlay -- container, video/
  audio bitrate, and every audio/subtitle track (not just the one
  selected), for any source, not just Plex. Hero layouts get one
  compact line; the plain banner/card layouts get the full per-track
  breakdown

* Mon Aug 24 2026 Iain Smith <iain@issinoho.com> - 1.19.0-1
- Add skip intro/credits for chaptered Plex VOD playback -- a small
  "Skip Intro"/"Skip Credits" prompt (confirmed with 'j') appears
  while playback is inside one of Plex's own intro/credits marker
  windows; --no-skip-markers disables it
- Add auto-play next episode for Plex TV shows -- an "Up Next" prompt
  with a cancellable countdown offers the next episode once the
  current one plays through to a genuine end; ESC cancels;
  --no-autoplay-next-episode disables it, --autoplay-countdown-seconds
  adjusts the countdown length

* Sun Aug 23 2026 Iain Smith <iain@issinoho.com> - 1.18.2-1
- Fix a crash-worthy ERROR-level traceback when is_paused races mpv
  shutdown -- the periodic playback-position autosave loop could hit
  this right as mpv's core was torn down mid-quit; now returns False
  cleanly instead of raising

* Sun Aug 23 2026 Iain Smith <iain@issinoho.com> - 1.18.1-1
- Suppress noisy "Multiple Dolby Vision RPUs found in one AU" ffmpeg
  warning spam in the log -- fires dozens of times a second on some
  Dolby Vision streams and has no bearing on actual decode
  correctness

* Sun Aug 23 2026 Iain Smith <iain@issinoho.com> - 1.18.0-1
- Skip between chapters on UP/DOWN for chaptered VOD playback (Plex
  only, for items with real chapter markers) -- "previous" jumps to
  the current chapter's own start unless already within 5s of it,
  mirroring standard DVD/Blu-ray remote behavior. Falls back to
  mpv's default 60s seek for anything without chapters;
  --no-chapter-skip keeps the old default seek everywhere
...
- Fix the Plex browser reopening on top of playback started from
  ...
  started playback correctly, but close_history_browser's own
  Plex-reopen logic (meant for backing out of history via ESC) fired
  regardless, popping the Plex browser back up over the now-playing
  video; play_selected_history_entry now suppresses that reopen
  whenever it's actually starting playback

* Sun Aug 23 2026 Iain Smith <iain@issinoho.com> - 1.16.1-1
- Fix Plex chapter markers never showing up in tvdinner --
  resolve_plex_playable fetched Plex's item metadata without the
  includeChapters=1 param, which Plex silently requires to include
  the Chapter array at all, even for an item whose own chapterSource
  field proves it has real chapter data; chapters showed fine in
  Plex's own clients but never made it into tvdinner
...
- Fix the guide's arrow keys getting stuck after a no-match filter --
  ...
  favorites-only view) emptied the eligible channel list, so every
  arrow-key press silently did nothing once that happened,
  permanently; now clears whichever filter emptied the list and falls
  back to what's left, same as the Plex browser's own favorites-only
  filter already did
...
- Bind MENU to the info overlay during bare local-file/YouTube
  ...
  live channel or Plex session, but was never bound at all here,
  silently falling through to mpv's own unused on-screen-select-
  script default instead

* Thu Aug 20 2026 Iain Smith <iain@issinoho.com> - 1.14.0-1
- Show TMDB title logos for Plex TV episodes too -- searches TMDB's
  /search/tv by the show's own name (Plex's grandparentTitle), instead
  of /search/movie by the episode's own title, which was never going
  to match. Logo art only; Plex already supplies its own backdrop for
  TV content

* Thu Aug 20 2026 Iain Smith <iain@issinoho.com> - 1.13.1-1
- Fix the TMDB title logo never showing for Plex playback -- main()'s
  Plex branch never passed the TMDB token into play_stream() at all,
  silently skipping the logo lookup every time; the VOD "now playing"
  popup also had no way to redraw itself once a background logo/
  backdrop lookup completed, the same class of bug already fixed for
  the live-channel EPG hero

* Thu Aug 20 2026 Iain Smith <iain@issinoho.com> - 1.13.0-1
- Show a TMDB title-treatment logo in the hero overlays -- fetched and
  cached alongside the existing backdrop art, composited in the
  top-right corner of both the live-channel EPG hero and the VOD/Plex
  info hero, backed by a scrim that adapts to white or black depending
  on the logo's own average lightness so it stays legible regardless
  of color. Plex items now also get a best-effort TMDB logo lookup
  layered on top of Plex's own backdrop art
...
- Add a long-press-ENTER item menu to the Plex browser -- "Play from
  ...
  Unwatched" against the selected movie, show, or episode; a normal
  tap still plays/drills in exactly as before
- Fix the Plex browser's full-screen poster backdrop to be fully
  opaque, instead of letting mpv's idle-screen logo bleed through it

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.10.4-1
- Replace the Plex browser's plain black root backdrop (which let
  mpv's own idle-screen logo show through) with a gentle gradient
  wash and tvdinner's own logo mark in the top-left corner

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.10.3-1
- Polish the Plex Grid view: the header bar now shows the selected
  item's trailing detail (subtitle or drill-in chevron), same as a
  List view row's own; toggling between Grid and List with 'g' now
  keeps the same item focused instead of jumping back to the first
  one

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.10.2-1
- Group Plex year-filter results by library, then sort within it --
  a movie library's results alphabetically by film name, a TV
  library's alphabetically by show then numerically by season and
  episode, instead of one flat alphabetical list mixing everything
  together

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.10.1-1
- Sharpen the Plex browser's poster backdrop -- reduce the Gaussian
  blur radius on both the full-screen and in-panel backdrops

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.10.0-1
- Add a full-screen poster backdrop to the Plex browser -- the
  currently selected movie/show's poster now fills the whole screen
  behind the browser panel (blurred and tinted, Netflix-style), in
  both Grid and List view

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.9.2-1
- Prefix Plex season breadcrumbs with the show name, e.g.
  "2 Broke Girls - Season 3" instead of just "Season 3"

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.9.1-1
...
- Make GO_BACK act like BS (stop and drop back into the library
  ...
  to mpv's own default binding (cycle fullscreen/window mode)
...
- Report the real base OS (X-Plex-Platform) and this machine's own
  ...
  "tvdinner" -- shown as Tautulli/Plex's Platform and Player columns

* Wed Aug 19 2026 Iain Smith <iain@issinoho.com> - 1.8.0-1
- Report Plex playback as a real session (Now Playing / Tautulli) --
  the same timeline API Plex's own clients use, so playback via
  tvdinner shows up in Plex's dashboard and updates its own watched
  status/resume position; on by default, --no-plex-activity disables
  it
- Rename the synthetic Plex root row from "Continue Watching" to "On
  Deck"
- Pausing now shows the same info overlay 'i'/MENU would (EPG banner
  or poster/synopsis/progress card) instead of just a plain "Paused"
  toast, auto-hiding itself as usual or clearing immediately on resume

* Tue Aug 18 2026 Iain Smith <iain@issinoho.com> - 1.7.0-1
- Add a synthetic Continue Watching row to the Plex library root,
  pulled from Plex's own server-wide /library/onDeck feed (movies
  left partway through, plus the next unwatched episode of any show
  you're partway through)
- Fix the Plex favorites-only view getting permanently stuck
  (unresponsive arrow keys/selection) whenever the filter emptied out
  an otherwise non-empty frame -- now falls back to the unfiltered
  list instead
